...
class JDFaissQuery_Ext(object):
    def __init__(self, faiss_index_file, faiss_sku_file, num_thread=10, log_instance=None):
        if not os.path.exists(faiss_index_file):
            raise Exception("%s does not exist!" % faiss_index_file)

        if not os.path.exists(faiss_sku_file):
            raise Exception("%s does not exist!" % faiss_sku_file)

        self.log_instance = log_instance
        self.faiss_index_file = faiss_index_file
        self.faiss_sku_file = faiss_sku_file
        self.num_thread = num_thread
        self.faiss_index_obj = self._getting_faiss()
        self.idx_sku_dict = self._loading_faiss_sku_from_file()

    def _getting_faiss(self):
        try:
            log_instance.info("begin loading faiss index: %s" % self.faiss_index_file)
            index = faiss.read_index(self.faiss_index_file)
            log_instance.info("end loading faiss index")
    ...
    def _loading_faiss_sku_from_file(self):
        idx_sku_dict = {}
        t = 0
        with open(self.faiss_sku_file) as f1:
            for line in f1:
                try:
                    sku = line.strip()
                    if line == '': continue
                    idx_sku_dict[t] = sku
                    if t % 10000 == 0:
                        log_instance.info("faiss-sku: %s" % t)
                    t += 1
                except:
                    self.log_instance.error(traceback.format_exc())
        return idx_sku_dict

    def _loading_query_data_from_file(self, query_vec_file):
        data = []
        sku_dict = {}
        idx = 0
        with open(query_vec_file) as f1:
            for line in f1:
                try:
                    sku, lst2 = self._line_dealing(line)
                    if sku == None or lst2 == None: continue
    ...
    def _faiss_search(self, query_np_data, topn=TOPN):
        try:
            log_instance.info("query-data rows:%s column:%s " % query_np_data.shape)
            log_instance.info("num_threads: %s, topn: %s" % (self.num_thread, topn))

            log_instance.info("begin query")
            faiss.omp_set_num_threads(self.num_thread)
            normalize_L2(query_np_data)
            dis, ind = self.faiss_index_obj.search(query_np_data, topn)
            log_instance.info("end query")
    ...
    def faiss_search_and_detail_info_saving(self, query_vec_file, query_idx_reuslt_saving_file, \
                                            query_sku_result_saving_file, topn=TOPN):
        """
        检索结果的保存
        :param query_vec_file:
        :param query_idx_reuslt_saving_file:
        :param query_sku_result_saving_file:
        :param topn:
        :return:
        """
        try:
            if not os.path.exists(query_vec_file):
                raise Exception("%s does not exist!" % query_vec_file)
            query_np_data, query_sku_dict = self._loading_query_data_from_file(query_vec_file)

            dis, ind = self._faiss_search(query_np_data, topn)
            self._query_result_detail_info(dis, ind, query_sku_dict, \
                                           query_idx_reuslt_saving_file, \
                                           query_sku_result_saving_file)
        except Exception as e:
            raise e

    def _query_result_detail_info(self, dis, ind, query_sku_dict, \
                                  query_idx_reuslt_saving_file, \
                                  query_sku_result_saving_file):
        try:
            score_lst = dis.tolist()
            ...
            r_lst2 = []
            for j in range(len(score_lst)):
                if j not in query_sku_dict: continue
                q_sku = query_sku_dict[j] if j in query_sku_dict else "unk0"

                sku_lst = []
                for z3 in idx_lst[j]:
                    if z3 not in self.idx_sku_dict:
                        sku_id = 'unk'
                    else:
                        sku_id = self.idx_sku_dict[z3]
            ...
            with open(query_idx_reuslt_saving_file, "w") as f2:
                f2.write("\n".join(r_lst2))
                f2.flush()

            with open(query_sku_result_saving_file, "w") as f3:
                f3.write("\n".join(r_lst1))
                f3.flush()
    ...
    def _IVFFlat_search(self, query_np_data):
        log_instance.info("query-data rows:%s column:%s " % self.query_np_data.shape)
        log_instance.info("num_threads: %s, topn: %s" % (self.num_thread, TOPN))
...
class AttachJdCat4Info2TmallSku(object):
    ...
    """
    def __init__(self, faiss_sku_info_folder, query_sku_info_folder, \
                 query_result_folder, attach_sku_info_saving_folder, \
                 log_instance):
        if not os.path.exists(faiss_sku_info_folder):
            raise Exception("%s does not exist!" % faiss_sku_info_folder)
        if not os.path.exists(query_sku_info_folder):
            raise Exception("%s does not exist!" % query_sku_info_folder)
        if not os.path.exists(query_result_folder):
            raise Exception("%s does not exist!" % query_result_folder)
        if not os.path.exists(attach_sku_info_saving_folder):
            raise Exception("%s does not exist!" % attach_sku_info_saving_folder)
        self.query_result_folder = query_result_folder
        self.attach_sku_info_saving_folder = attach_sku_info_saving_folder
        self.log_instance = log_instance

        self.faiss_sku_info_dict = self._loading_faiss_sku_info(faiss_sku_info_folder)
        self.query_sku_info_dict = self._loading_query_sku_info(query_sku_info_folder)
        if len(self.faiss_sku_info_dict) == 0:
            raise Exception("faiss-sku-info is empty! folder: %s" % faiss_sku_info_folder)
        if len(self.query_sku_info_dict) == 0:
            raise Exception("query-sku-info is empty! folder: %s" % query_sku_info_folder)

    def _loading_faiss_sku_info(self, data_folder):
    ...
    def _loading_query_sku_info(self, data_folder):
        file_lst = getting_sorted_file_name_lst(data_folder)
        r_dict = {}
        idx = 0
        for fname in file_lst:
    ...
    def _attach_sku_info(self, itm_lst):
        ...
        if t_sku not in self.query_sku_info_dict:
            t_title = ""
        else:
            t_title = self.query_sku_info_dict[t_sku]
        idx  = 0
        for tmp in re_sku.strip().split(','):
            ...
            j_sku, j_score = lst1
            if j_sku not in self.faiss_sku_info_dict:
                continue
            idx += 1
            j_title, jd_cat4_id, jd_cat4_name = self.faiss_sku_info_dict[j_sku]
    ...
    def attach_sku_info(self):
        file_lst = getting_sorted_file_name_lst(self.query_result_folder)
        idx = 1
        for fname in file_lst:
            r_lst = []
            fname = fname.split("/")[-1]
            if not fname.endswith("_sku.result"): continue
            f_path = self.query_result_folder + "/" + fname
            if not os.path.exists(f_path):
                self.log_instance.error("%s file does not exists!" % f_path)
                continue
            ...
            out_file = self.attach_sku_info_saving_folder + "/" + fname
            with open(out_file, "w") as f2:
                f2.write("\n".join(r_lst))
                f2.flush()


class AttachJdCat4Info2TmallSkuWithCat3Filtering(object):
    """
    为天猫的【食品饮料】的商品获取四级类目信息
    """
    def __init__(self, query_result_folder, \
                 faiss_sku_info_folder, query_sku_info_folder, \
                 attach_sku_info_saving_folder, log_instance):
        if not os.path.exists(query_result_folder):
            raise Exception("%s does not exist!" % query_result_folder)
        if not os.path.exists(faiss_sku_info_folder):
            raise Exception("%s does not exist!" % faiss_sku_info_folder)
        ...
        self.query_sku_info_dict = self._loading_query_sku_info(query_sku_info_folder)
        if len(self.faiss_sku_info_dict) == 0:
            raise Exception("self.faiss_sku_info_dict is empty!")
        if len(self.query_sku_info_dict) == 0:
            raise Exception("self.query_sku_info_dict is empty!")
        ...
        for fname in file_lst:
            ...
            with open(f_path) as f1:
                for line in f1:
                    ...
                    if line == "": continue
                    lst1 = line.split("\t")
                    if len(lst1) != 6: continue
                    idx += 1
                    if idx % 100000 == 0:
                        self.log_instance.info("loading jd-cat4-sku-info: %s" % idx)
                    lst1 = [tmp.strip() for tmp in lst1]
                    sku, title, cat3_id, cat3_name, cat4_id, cat4_name = lst1
                    r_dict[sku] = (title, cat3_id, cat3_name, cat4_id, cat4_name)
        return r_dict
            ...
            with open(f_path) as f1:
                for line in f1:
                    ...
                    if idx % 100000 == 0:
                        self.log_instance.info("loading tmall-sku-info: %s" % idx)
                    lst1 = [tmp.strip() for tmp in lst1]
                    sku, title, cat3_id, cat3_name = lst1
                    r_dict[sku] = (title, cat3_id, cat3_name)
        return r_dict

    def _is_same_cat3(self, t_sku, j_sku):
        t_cat3_id, t_cat3_name = "", ""
        if t_sku not in self.query_cat3_dict:
            return False

        if j_sku not in self.faiss_cat3_dict:
            return False

        t_c3_id, t_c3_name = self.query_cat3_dict[t_sku]
        j_c3_id, j_c3_name = self.faiss_cat3_dict[j_sku]
        if t_c3_id == j_c3_id:
            return True
        else:
            return False
    ...
    def _attach_sku_info(self, itm_lst):
        """
        三份数据：
            part1(4): tmall_sku tmall_title tmall_c3id tmall_c3name
            part2(6): jd_sku1 jd_title1 jd_c3id1 jd_c3name1 jd_c4id1 jd_c4name1
            part3(2): score1 idx1
            part4(2): re_str d_type
            第一份：所有数据, d_type="all_data"
            第二份：原始top1的数据, d_type="ori_top1"
            第三份：cat3的top1的数据, d_type="cat3_top1"

        :param itm_lst:
        :return:
        """
        tpl_str = "%s\001%s\001%s\001%s\001%s\001%s\001%s\001%s\001%s\001%s\001%s\001%s"
        r_lst1 = []
        t1_dict = {}
        cat_t1_dict = {}
        t_sku, re_sku = itm_lst
        if t_sku not in self.query_sku_info_dict:
            t_title, t_c3id, t_c3name = "", "", ""
        else:
            t_title, t_c3id, t_c3name = self.query_sku_info_dict[t_sku]
        idx  = 0
        r_sku_lst = re_sku.strip().split(',')
        for z in range(len(r_sku_lst)):
            tmp = r_sku_lst[z].strip()
            if tmp == "": continue
            lst1 = tmp.split(":")
            if len(lst1) != 2: continue
            ...
            if j_sku not in self.faiss_sku_info_dict:
                continue
            j_title, j_c3id, j_c3name, j_c4id, j_c4name = self.faiss_sku_info_dict[j_sku]

            idx += 1
            tpl_line =tpl_str % (t_sku, t_title, t_c3id, t_c3name, \
                                 j_sku, j_title, j_c3id, j_c3name, j_c4id, j_c4name, \
                                 j_score, idx)
            if idx == 1 and t_sku not in t1_dict:
                t1_dict[t_sku] = tpl_line + "\001%s\001%s" % (re_sku, "ori_top1")
            if t_c3id == j_c3id and t_sku not in cat_t1_dict:
                cat_t1_dict[t_sku] = tpl_line + "\001%s\001%s" % (re_sku, "cat3_top1")
            # tmall_sku tmall_title jd_sku1 jd_title1 score1 idx1
            r_lst1.append(tpl_line + "\001%s\001%s" % (re_sku, "all_data"))

        r_lst2 = [v1 for k1, v1 in t1_dict.items()]
        r_lst3 = [v2 for k2, v2 in cat_t1_dict.items()]

        return r_lst1, r_lst2, r_lst3
            ...
            with open(f_path) as f1:
                for line in f1:
                    ...
                    if idx % 100000 == 0:
                        self.log_instance.info("attach-sku-info: %s" % idx)
                    lst1 = [tmp.strip() for tmp in lst1]
                    l1, l2, l3 = self._attach_sku_info(lst1)
                    r_lst1 += l1
                    r_lst2 += l2
                    r_lst3 += l3

            all_data_file = self.attach_sku_info_saving_folder + "/" + fname
            with open(all_data_file, "w") as f2:
                f2.write("\n".join(r_lst1))
                f2.flush()

            ori_top1_file = self.attach_sku_info_saving_folder + "/" + fname + ".ori_top1"
            with open(ori_top1_file, "w") as f3:
                f3.write("\n".join(r_lst2))
                f3.flush()

            cat3_top1_file = self.attach_sku_info_saving_folder + "/" + fname + ".cat3_top1"
            with open(cat3_top1_file, "w") as f4:
                f4.write("\n".join(r_lst3))
                f4.flush()
